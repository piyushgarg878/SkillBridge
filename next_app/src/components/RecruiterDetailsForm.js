...
export default function RecruiterDetailsForm({ userId }) {
  const [form, setForm] = useState({ name: '', age: '', company: '' });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.name || !form.age || !form.company) {
    ...
    try {
      const res = await fetch('/api/onboard/recruiter', {
        ...
        body: JSON.stringify({ userId, name: form.name, age: form.age, companyName: form.company })
      });
